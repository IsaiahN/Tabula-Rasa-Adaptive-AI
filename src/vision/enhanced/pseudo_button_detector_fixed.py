    ...
    def calculate_frame_differences(self, frame_before: List[List[int]],
        ...
        for y in range(height):
            for x in range(width):
                try:
                    # Extract numeric values from cells using the helper method
                    value_before = self._extract_cell_value(frame_before[y][x])
                    value_after = self._extract_cell_value(frame_after[y][x])
                    
                    # Now we can safely subtract the numeric values
