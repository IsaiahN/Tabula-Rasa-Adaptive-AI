...
class Architect:
    ...
    def __init__(self, base_path: str, repo_path: str, logger: Optional[logging.Logger] = None):
        ...
        # Git integration
        self.repo = None
        self.default_branch = "Arc3-AGI-Testing"  # Our working branch
        if GIT_AVAILABLE:
            try:
    ...
    def _ensure_correct_branch(self):
        """Ensure we're on the Arc3-AGI-Testing branch and never switch to main/master."""
        if not self.repo:
            return
    ...
    def _safe_checkout_default_branch(self):
        """Safely checkout the Arc3-AGI-Testing branch, never main/master."""
        if not self.repo:
            return False
...
"""
            ...
            # Return to Arc3-AGI-Testing branch (our default working branch)
...
"""
            ...
            # Try to return to Arc3-AGI-Testing branch (our default working branch)
            self._safe_checkout_default_branch()
