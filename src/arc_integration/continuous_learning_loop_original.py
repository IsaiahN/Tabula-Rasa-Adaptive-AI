...
try:
    ...
    from src.arc_integration.database_bounded_list import DatabaseBoundedList, DatabaseBoundedDict
except ImportError:
...
class ContinuousLearningLoop:
    ...
    def _initialize_complex_attributes(self):
        ...
        # Initialize performance tracking with database storage
        self.performance_history = DatabaseBoundedList(max_size=100, table_name="performance_history")
        self.session_history = DatabaseBoundedList(max_size=100, table_name="session_history")
    ...
    async def cleanup_sessions(self):
        ...
        # Training state
        self.current_session: Optional[TrainingSession] = None
        self.session_history: DatabaseBoundedList = DatabaseBoundedList(max_size=100, table_name="session_history")
        self.performance_history: DatabaseBoundedList = DatabaseBoundedList(max_size=100, table_name="performance_history")  # Add performance history for Governor/Architect
        self.global_training_sessions = {
            'total_games_played': 0,
            'total_episodes': 0,
            'average_score': 0.0,
        ...
        try:
            ...
            from src.utils.streamlined_output import get_streamlined_output, OutputLevel
        ...
        self._progress_tracker = {
            ...
            'termination_reason': None,
            'explored_coordinates': [],  # Track coordinate exploration
    ...
    def _check_progress_and_extend_cap(self, current_score: int, actions_taken: int, current_cap: int, max_actions_per_game: int) -> int:
        ...
        if len(tracker.get('score_history', [])) >= 2:
            ...
            if len(recent_scores) >= 2 and recent_scores[-1] > recent_scores[0]:
                ...
                from src.utils.streamlined_output import get_streamlined_output, OutputLevel
                output = get_streamlined_output()
                output._print(f"📈 Score: {recent_scores[0]} → {recent_scores[-1]}", OutputLevel.MINIMAL, output.Fore.GREEN)
        ...
        if tracker.get('last_meaningful_change', 0) > 0:
            ...
            if actions_since_change < 100:  # Recent change within last 100 actions
                ...
                if actions_since_change < 20:
                    from src.utils.streamlined_output import get_streamlined_output, OutputLevel
                    output = get_streamlined_output()
                    output._print(f"🔄 Recent change ({actions_since_change} actions ago)", OutputLevel.NORMAL, output.Fore.BLUE)
        ...
        if stagnation_actions < 50:  # Less than 50 actions without progress
            ...
            if stagnation_actions < 10:
                from src.utils.streamlined_output import get_streamlined_output, OutputLevel
                output = get_streamlined_output()
                output._print(f"⚡ Active progress ({stagnation_actions} actions without progress)", OutputLevel.NORMAL, output.Fore.GREEN)
        ...
        if recent_progress and current_cap < max_actions_per_game:
            ...
            if len(tracker.get('score_history', [])) >= 3:
                ...
                if recent_scores[-1] > recent_scores[0] and (recent_scores[-1] - recent_scores[0]) > 5:
                    ...
                    from src.utils.streamlined_output import get_streamlined_output, OutputLevel
                    output = get_streamlined_output()
                    output._print(f"🚀 Strong progress: +{recent_scores[-1] - recent_scores[0]} points", OutputLevel.MINIMAL, output.Fore.GREEN)
            ...
            if current_cap >= int(max_actions_per_game * 0.8):  # Already at 80% of max
                ...
                from src.utils.streamlined_output import get_streamlined_output, OutputLevel
                output = get_streamlined_output()
                output._print(f"⚡ Extending to {max_actions_per_game} actions", OutputLevel.NORMAL, output.Fore.YELLOW)
            ...
            if new_cap > current_cap:
                from src.utils.streamlined_output import get_streamlined_output, OutputLevel
                output = get_streamlined_output()
                output._print(f"📈 Cap: {current_cap} → {new_cap} actions", OutputLevel.NORMAL, output.Fore.CYAN)
                return new_cap
    ...
    async def _start_game_session(self, game_id: str, existing_guid: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if existing_guid:
                ...
                from src.utils.streamlined_output import get_streamlined_output, OutputLevel
                output = get_streamlined_output()
                output._print(f"🔄 Level reset: {game_id[:8]}", OutputLevel.MINIMAL, output.Fore.BLUE)
            else:
                # New Game - start fresh game session
                from src.utils.streamlined_output import get_streamlined_output, OutputLevel
                output = get_streamlined_output()
                output._print(f"🎮 New game: {game_id[:8]}", OutputLevel.MINIMAL, output.Fore.GREEN)
        ...
        if hasattr(self, 'current_scorecard_id') and self.current_scorecard_id:
            try:
                ...
                try:
                    ...
                    # Instead, just mark it for closure and let the main loop handle it
                    print(f" ⚠️ Cannot close scorecard synchronously - marking for closure: {self.current_scorecard_id}")
                    self._force_scorecard_close = True
                except RuntimeError:
            ...
            if 'scorecard_id' in session_data and session_data['scorecard_id']:
                try:
                    try:
                        ...
                        # We're in an event loop, so we can't use run_until_complete
                        # Instead, just mark it for closure and let the main loop handle it
                        print(f" ⚠️ Cannot close scorecard synchronously - marking for closure: {session_data['scorecard_id']}")
                        self._force_scorecard_close = True
                    except RuntimeError:
                        # No event loop running, create a new one
                        loop = asyncio.new_event_loop()
    ...
    def _check_scorecard_action_limit(self) -> bool:
        ...
        if self._scorecard_action_count >= self._max_actions_per_scorecard:
            output._print(f"⚠️ Scorecard limit: {self._scorecard_action_count}/{self._max_actions_per_scorecard}", OutputLevel.MINIMAL, output.Fore.RED)
            return True
        return False
    ...
    def _parse_episode_results_comprehensive(self, stdout: str, stderr: str, game_id: str) -> Dict[str, Any]:
        ...
        for pattern in full_game_win_patterns:
            if re.search(pattern, combined_output, re.IGNORECASE):
                ...
                from src.utils.streamlined_output import get_streamlined_output, OutputLevel
                output = get_streamlined_output()
                output._print(f"🏆 FULL GAME WIN!", OutputLevel.MINIMAL, output.Fore.GREEN)
                break
        ...
        if not result['success']:
            ...
            if result['final_score'] > 0:
                ...
                from src.utils.streamlined_output import get_streamlined_output, OutputLevel
                output = get_streamlined_output()
                output._print(f"🎯 Score win: {result['final_score']} points", OutputLevel.MINIMAL, output.Fore.GREEN)
            ...
            # If we have many actions taken without explicit failure, consider it progress
                ...
                from src.utils.streamlined_output import get_streamlined_output, OutputLevel
                output = get_streamlined_output()
                output._print(f"🎯 Progress win: {result['total_actions']} actions", OutputLevel.MINIMAL, output.Fore.GREEN)
    ...
    def _safe_coordinate_fallback(self, grid_width: int, grid_height: int, reason: str = "bounds check failed") -> Tuple[int, int]:
        ...
        output._print(f"🔄 Fallback: ({center_x},{center_y}) - {reason}", OutputLevel.NORMAL, output.Fore.YELLOW)
    ...
    def _select_coordinate_based_action(self, response_data: Dict[str, Any], game_id: str) -> int:
        ...
        if coordinate_stagnation['should_switch']:
            ...
            if coordinate_stagnation['recommended_coordinates']:
                ...
                from src.utils.streamlined_output import get_streamlined_output, OutputLevel
                output = get_streamlined_output()
                output._print(f"🎯 Selected: ({x},{y})", OutputLevel.NORMAL, output.Fore.CYAN)
        else:
            ...
            try:
                ...
                from src.utils.streamlined_output import get_streamlined_output, OutputLevel
                output = get_streamlined_output()
                output._print(f"⚠️ Coordinate selection failed: {e}", OutputLevel.NORMAL, output.Fore.RED)
                # Fallback to systematic exploration
                x, y = self._generate_exploration_coordinates((grid_width, grid_height), game_id)
                output._print(f"🎯 Fallback: ({x},{y})", OutputLevel.NORMAL, output.Fore.YELLOW)
                                ...
                                if action_number == 6 and x is not None and y is not None and hasattr(self, 'frame_analyzer') and hasattr(self.frame_analyzer, 'log_action6_interaction'):
                                    try:
                                        ...
                                        if frame_changed:
                                            ...
                                            try:
                                                from src.database.performance_data_manager import PerformanceDataManager
                                                performance_manager = PerformanceDataManager()
                                                frame_data = {
                                                    'frame_hash': str(hash(str(data.get('frame', [])))),
                                                    'frame_analysis': frame_changed,
                                                    'stagnation_detected': False
                                                }
                                                try:
                                                    loop = asyncio.get_event_loop()
                                                    if loop.is_running():
                                                        task = asyncio.create_task(performance_manager.store_frame_tracking(game_id, frame_data))
                                                        if not hasattr(self, '_pending_tasks'):
                                                            self._pending_tasks = []
                                                        self._pending_tasks.append(task)
                                                    else:
                                                        asyncio.run(performance_manager.store_frame_tracking(game_id, frame_data))
                                                except RuntimeError:
                                                    pass
                                            except Exception as e:
                                                logger.error(f"Failed to save frame tracking: {e}")
                ...
                if hasattr(self, 'frame_analyzer') and hasattr(self.frame_analyzer, 'mark_color_explored'):
                    ...
                    if frame_valid:
                        try:
                            ...
                            output._print(f"🔍 {status} Color {clicked_color} at ({x},{y})", OutputLevel.NORMAL, output.Fore.CYAN)
                        except Exception as e:
                            from src.utils.streamlined_output import get_streamlined_output, OutputLevel
                            output = get_streamlined_output()
                            output._print(f"⚠️ Exploration error: {e}", OutputLevel.NORMAL, output.Fore.RED)
                    else:
                    ...
                    try:
                        ...
                        try:
                            ...
                            performance_manager = PerformanceDataManager()
                            action_data = {
                                'action_type': f'action_{action_number}',
                                'action_sequence': [action_number],
                                'effectiveness': 1.0 if data.get('frame_changed', False) else 0.0,
                                'context': {
                                    'coordinates': {'x': x, 'y': y} if action_number == 6 else None,
                                    'score_change': data.get('score', 0) - current_score if isinstance(data, dict) else 0,
                                    'frame_changed': data.get('frame_changed', False)
                                }
                            }
                            try:
                                loop = asyncio.get_event_loop()
                                if loop.is_running():
                                    task = asyncio.create_task(performance_manager.store_action_tracking(game_id, action_data))
                                    if not hasattr(self, '_pending_tasks'):
                                        self._pending_tasks = []
                                    self._pending_tasks.append(task)
                                else:
                                    asyncio.run(performance_manager.store_action_tracking(game_id, action_data))
                            except RuntimeError:
                                pass
                        except Exception as e:
                            logger.error(f"Failed to save action tracking: {e}")
                            ...
                            try:
                                ...
                                try:
                                    loop = asyncio.get_event_loop()
                                    if loop.is_running():
                                        task = asyncio.create_task(integration.log_error(
                                            "action_tracking_error",
                                            str(e),
                                            context=f"game_id={game_id}, action_number={action_number}"
                                        ))
                                        if not hasattr(self, '_pending_tasks'):
                                            self._pending_tasks = []
                                        self._pending_tasks.append(task)
                    ...
                    try:
                        if response.status == 200:
                            ...
                            # Await any pending async tasks to ensure data is saved immediately
        ...
        while session_count < max_mastery_sessions:  # Renamed from max_episodes
            try:
                ...
                if session_result and 'error' not in session_result:  # Updated variable name
                    ...
                    # Save score to score_history table
                    try:
                        from src.database.performance_data_manager import PerformanceDataManager
                        performance_manager = PerformanceDataManager()
                        score_data = {
                        ...
                        try:
                            loop = asyncio.get_event_loop()
                            if loop.is_running():
                                task = asyncio.create_task(performance_manager.store_score_data(game_id, score_data))
                                if not hasattr(self, '_pending_tasks'):
                                    self._pending_tasks = []
                                self._pending_tasks.append(task)
                            else:
                                asyncio.run(performance_manager.store_score_data(game_id, score_data))
                        except RuntimeError:
                            pass
                    except Exception as e:
                        logger.error(f"Failed to save score data: {e}")
                    ...
                    if session_result.get('full_game_win', False):
                        from src.utils.streamlined_output import get_streamlined_output, OutputLevel
                        output = get_streamlined_output()
                        output._print(f"🏆 FULL GAME WIN! {game_id[:8]}", OutputLevel.MINIMAL, output.Fore.GREEN)
                        success = True
                    ...
                    #  CHECK FOR LEVEL PROGRESSION - Only preserve NEW breakthroughs
                        ...
                        if new_level > previous_best:
                            from src.utils.streamlined_output import get_streamlined_output, OutputLevel
                            output = get_streamlined_output()
                            output._print(f"🎯 Level {previous_best} → {new_level}", OutputLevel.MINIMAL, output.Fore.GREEN)
                            ...
                            from src.utils.streamlined_output import get_streamlined_output, OutputLevel
                            output = get_streamlined_output()
                            output._print(f"🎯 Level {new_level} win!", OutputLevel.MINIMAL, output.Fore.GREEN)
                            ...
                            output._print(f"Level {new_level} maintained", OutputLevel.NORMAL, output.Fore.BLUE)
    ...
    def _update_coordinate_intelligence(self, x: int, y: int, game_id: str, success: bool):
        ...
        try:
            ...
            try:
                ...
                async def save_coordinate_intelligence_async():
                try:
                        ...
                        logger.info(f"Saved coordinate intelligence for {game_id} at ({x},{y})")
                    except Exception as e:
                        logger.error(f"Failed to save coordinate intelligence: {e}")
                ...
                try:
                    ...
                    if loop.is_running():
                        # Schedule the coroutine to run in the background
                        task = asyncio.create_task(save_coordinate_intelligence_async())
                        ...
                        asyncio.run(save_coordinate_intelligence_async())
                except RuntimeError:
    ...
    def _track_coordinate_selection(self, x: int, y: int, game_id: str, method: str):
        ...
        try:
            if not hasattr(self, '_coordinate_tracking'):
                self._coordinate_tracking = DatabaseBoundedDict(max_size=500, table_name="coordinate_tracking", game_id=game_id)
    ...
    def _should_reset_due_to_stagnation(self, session_result: Dict[str, Any], game_id: str) -> bool:
        ...
        if not hasattr(self, 'frame_stagnation_tracker'):
            self.frame_stagnation_tracker = DatabaseBoundedDict(max_size=100, table_name="frame_tracking", game_id=game_id)
    ...
    def _save_winning_sequence(self, game_id: str, winning_sequence: List[int]):
        ...
        try:
            ...
            # Save winning sequence directly to winning_sequences table
            import json
            
            async def save_winning_sequence_async():
                try:
                    # First save to learned_patterns table
                    ...
                    # Also save to winning_sequences table
                    from src.database.api import get_database
                    db = get_database()
                    
                    # Check if sequence already exists
                    existing = await db.fetch_all(
                        "SELECT id FROM winning_sequences WHERE game_id = ? AND sequence = ?",
                        (game_id, json.dumps(winning_sequence))
                    )
                    
                    if not existing:
                        # Insert new winning sequence
                        await db.execute("""
                        ...
                        """, (
                            game_id,
                            json.dumps(winning_sequence),
                            1,
                            0.0,  # Will be updated when we have score data
                            1.0,
                            time.time()
                        ))
                        logger.info(f"Saved winning sequence for {game_id}: {winning_sequence}")
                    else:
                        # Update frequency
                        await db.execute("""
                        ...
                        """, (time.time(), game_id, json.dumps(winning_sequence)))
                        logger.info(f"Updated frequency for winning sequence for {game_id}: {winning_sequence}")
                        
                except Exception as e:
                    logger.error(f"Failed to save winning sequence to database: {e}")
                    import traceback
                    logger.error(f"Traceback: {traceback.format_exc()}")
            ...
            try:
                ...
                if loop.is_running():
                    # Schedule the coroutine to run in the background
                    task = asyncio.create_task(save_winning_sequence_async())
                    # Add to pending tasks for later awaiting
                    if not hasattr(self, '_pending_tasks'):
                        self._pending_tasks = []
                    self._pending_tasks.append(task)
                    ...
                    # Run directly if no event loop is running
                    asyncio.run(save_winning_sequence_async())
            except RuntimeError:
                # Skip if event loop issues
                pass
                
                except Exception as e:
            logger.error(f"Failed to save winning sequence: {e}")
                    import traceback
                    logger.error(f"Traceback: {traceback.format_exc()}")
    ...
    async def _await_pending_tasks(self):
        ...
        if hasattr(self, '_pending_tasks') and self._pending_tasks:
            try:
                print(f"🔄 DEBUG: Awaiting {len(self._pending_tasks)} pending async tasks...")
                ...
                for i, result in enumerate(results):
                    if isinstance(result, Exception):
                        print(f"❌ DEBUG: Task {i} failed: {result}")
                        logger.error(f"Task {i} failed: {result}")
                self._pending_tasks.clear()
                print(f"✅ DEBUG: All {len(results)} pending tasks completed")
                logger.info("All pending tasks completed")
            except Exception as e:
                print(f"❌ DEBUG: Error awaiting pending tasks: {e}")
                logger.error(f"Error awaiting pending tasks: {e}")
        else:
            print(f"🔄 DEBUG: No pending tasks to await (hasattr: {hasattr(self, '_pending_tasks')}, tasks: {getattr(self, '_pending_tasks', None)})")
    ...
    def _update_scorecard_stats(self):
        ...
        try:
            ...
            if scorecard_data:
                ...
                from src.utils.streamlined_output import get_streamlined_output, OutputLevel
                output = get_streamlined_output()
                output._print(f"📊 Scorecard: {analysis['level_completions']} levels, {analysis['games_completed']} games", OutputLevel.NORMAL, output.Fore.CYAN)
                
        except Exception as e:
            from src.utils.streamlined_output import get_streamlined_output, OutputLevel
            output = get_streamlined_output()
            output._print(f"⚠️ Scorecard error: {e}", OutputLevel.NORMAL, output.Fore.RED)
    
    def _log_level_completion(self, game_id: str, level: int, score: int):
        """Log a level completion event."""
        from src.utils.streamlined_output import get_streamlined_output, OutputLevel
        output = get_streamlined_output()
        output._print(f"🎯 Level {level} completed (Score: {score})", OutputLevel.MINIMAL, output.Fore.GREEN)
    ...
    async def start_training_with_direct_control(
        ...
        try:
            ...
            while (actions_taken < actual_max_actions and 
                ...
                from src.utils.streamlined_output import get_streamlined_output, OutputLevel
                output = get_streamlined_output()
                output._print(f"Action {selected_action}{coord_display}", OutputLevel.MINIMAL, output.Fore.CYAN)
                ...
                if efficiency_metrics.get('optimization_count', 0) > 0:
                    ...
                    if total_saved > 10:  # Only show if significant savings
                        output._print(f"⚡ Saved {total_saved} moves", OutputLevel.NORMAL, output.Fore.GREEN)
                ...
                if "Learned:" in action_desc:
                    ...
                    if len(learned_info) > 50:  # Only show if significant learning
                        output._print(f"Learned: {learned_info[:50]}...", OutputLevel.NORMAL, output.Fore.YELLOW)
                    ...
                    if total_attempts > 5:  # Only show if significant attempts
                        ...
                        output._print(f"Success: {success_rate:.1%} ({total_attempts} tries)", OutputLevel.NORMAL, output.Fore.BLUE)
                ...
                if action_result:
                    # 🧠 SIMPLE ERROR HANDLING - Log API errors
                    if 'error' in action_result:
                        print(f"🧠 API ERROR: {action_result.get('error', 'Unknown error')}")
                        
                        # Simple error handling - just log and continue
                        if 'GAME_NOT_STARTED_ERROR' in str(action_result.get('error', '')):
                            print(f"🔄 GAME NOT STARTED - attempting to continue")
                        elif 'timeout' in str(action_result.get('error', '')).lower():
                            print(f"⏰ TIMEOUT ERROR - continuing with next action")
                        else:
                            print(f"⚠️ OTHER ERROR - continuing with next action")
            ...
            try:
                ...
                # 🏗️ ARCHITECT INTEGRATION - Simple performance tracking
                if self.architect:
                    try:
                        # Simple performance tracking without complex analysis
                        if final_result.get('success', False) and current_score > 0:
                            print(f"🏗️ ARCHITECT: Session successful with score {current_score}")
                        elif actions_taken > 0:
                            print(f"🏗️ ARCHITECT: Session completed with {actions_taken} actions")
                    except Exception as e:
                ...
                print(f"📊 GLOBAL COUNTERS: Session completed, total sessions: {self.global_counters.get('total_sessions', 0)}")
                    ...
                    try:
                        ...
                        performance_data = {
                            ...
                            'learning_efficiency': len(effective_actions) / max(1, actions_taken),
                            'metadata': {
                                'actions_taken': actions_taken,
                                'effective_actions': len(effective_actions),
                                'final_state': final_result.get('termination_reason', 'UNKNOWN')
                            }
                        }
                        try:
                            loop = asyncio.get_event_loop()
                            if loop.is_running():
                                task = asyncio.create_task(performance_manager.store_performance_data(
                                    getattr(self, 'session_id', 'continuous_learning'), 
                                    performance_data
                                ))
                                if not hasattr(self, '_pending_tasks'):
                                    self._pending_tasks = []
                                ...
                                asyncio.run(performance_manager.store_performance_data(
                                    getattr(self, 'session_id', 'continuous_learning'), 
                                    performance_data
                                ))
                        except RuntimeError:
            ...
            try:
                ...
                try:
                    ...
                    session_data = {
                        'game_id': game_id,
                        'status': final_result.get('termination_reason', 'UNKNOWN'),
                        'duration_seconds': int(time.time() - getattr(self, '_session_start_time', time.time())),
                        'actions_taken': actions_taken,
                        'score': current_score,
                        'win': final_result.get('success', False),
                        'metadata': {
                            'effective_actions': len(effective_actions),
                            'final_state': current_state
                        }
                    ...
                    try:
                        loop = asyncio.get_event_loop()
                        if loop.is_running():
                            task = asyncio.create_task(performance_manager.store_session_data(
                                getattr(self, 'session_id', 'continuous_learning'), 
                                session_data
                            ))
                            if not hasattr(self, '_pending_tasks'):
                            ...
                            asyncio.run(performance_manager.store_session_data(
                                getattr(self, 'session_id', 'continuous_learning'), 
                                session_data
                            ))
                    except RuntimeError:
    ...
    async def _fallback_sleep_result(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self, 'frame_analyzer') and self.frame_analyzer:
                ...
                if hasattr(self, '_progress_tracker'):
                    explored_coords = getattr(self._progress_tracker, 'explored_coordinates', [])
